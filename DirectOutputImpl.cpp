...
namespace {
	void ReportError(LPCWSTR message) {
	...
	m_savefile(0), m_displayfile(0), m_deletefile(0)
{
	...
	if (GetDirectOutputFilename(filename, sizeof(filename) / sizeof(filename[0])))
	{
		...
		if (m_module)
		{
			...
			ReportError(_T("DLL load error"));
		}
	} else {
		ReportError(_T("Get filename error"));
	}
}
CDirectOutput::~CDirectOutput()
{
...
HRESULT CDirectOutput::GetSerialNumber(void* hDevice, wchar_t* pszSerialNumber, DWORD dwSize)
{
...
bool CDirectOutput::GetDirectOutputFilename(LPTSTR filename, DWORD length)
{
	...
	if (ERROR_SUCCESS == lRet)
	{
		...
		lRet = RegQueryValueEx(hk, _T("DirectOutput_Saitek"), 0, 0, (LPBYTE)filename, &size);
		if (ERROR_SUCCESS == lRet)
		{
			retval = true;
		} else {
			ReportError(_T("can't find reg key DirectOutput_Saitek"));
		}
		RegCloseKey(hk);
	} else {
		ReportError(_T("can't find reg directory HKLM\\SOFTWARE\\Saitek\\DirectOutput"));
	}
	return retval;
}
